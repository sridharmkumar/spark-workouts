...
object SparkRevision {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("*** Case #4 - Complete ***")

    println
    println("*** Case #5 - Start ***")
    val fileData02 = sc.textFile("file:///c:/data/file2.txt")
    ...
    rowDataFrame.show(5)
    println("*** Case #6 - Complete ***")

    println
    println("*** Case #7 - Start ***")
    val csvDataFrame = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("file:///c:/data/file3.txt").select(columnList.map(col): _*)
    csvDataFrame.show(5, truncate = false)
    println("*** Case #7 - Complete ***")

    println
    println("*** Case #8 - Start ***")
    val jsonDataFrame = spark.read.format("json").load("file:///c:/data/file4.json").select(columnList.map(col): _*)
    jsonDataFrame.show(5, truncate = false)

    val parquetDataFrame = spark.read.format("parquet").load("file:///c:/data/file5.parquet").select(columnList.map(col): _*)
    parquetDataFrame.show(5, truncate = false)
    println("*** Case #8 - Complete ***")

    println
    println("*** Case #9 - Start ***")
    val xmlDataFrame = spark.read.format("xml").option("rowTag", "txndata").load("file:///c:/data/file6").select(columnList.map(col): _*)
    xmlDataFrame.show(5, truncate = false)
    println("*** Case #9 - Complete ***")

    println
    println("*** Case #10 - Start ***")
    ...
    println("*** Case #10 - Complete ***")

    println
    println("*** Case #11 - Start ***")
    ...
    println("*** Case #11 - Complete ***")

    println
    println("*** Case #12 - Start ***")
    finalData.write.format("com.databricks.spark.avro").partitionBy("category").mode("append").save("file:///c:/data/output/union_data")
    println("Final data written successfully")
    println("*** Case #12 - Complete ***")
  }
}
